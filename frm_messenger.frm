...
Private Sub setup_message(pos As Integer)
    ...
    back_colour = wm_colours(wm_colvars(colvar_page).variable_colour).web_colour
    If back_colour = "" Then back_colour = wm_colours(wm_White).web_colour
        Select Case pos
...
Private Sub write_message(message As String, recieved As Boolean)
    ...
    If recieved = True Then
        text_colour = wm_colours(wm_colvars(colvar_rxmsg).variable_colour).web_colour
        If text_colour = "" Then text_colour = wm_colours(wm_Red).web_colour
        snd_events(event_onrx).start
    Else
        text_colour = wm_colours(wm_colvars(colvar_txmsg).variable_colour).web_colour
        If text_colour = "" Then text_colour = wm_colours(wm_Blue).web_colour
